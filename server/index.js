...
async function tryOllamaGenerate({ code, energyKw }) {
  ...
  const prompt = `You are a GreenOps advisor.\n\nGiven:\n- Energy estimate: ${energyKw} kW\n- Code snippet:\n${code}\n\nTask:\n1) Identify energy-heavy parts.\n2) Propose concrete refactors/scheduling suggestions.\n3) Keep response under 200 words.\n\nReturn ONLY a short recommendation paragraph.`;

  try {
    const r = await axios.post(`${baseUrl}/api/generate`, { model, prompt, stream: false }, { timeout: 6000 });
